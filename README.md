...
3. 双击 `run_local.bat`。首次会自动装依赖（较慢），并**弹一次浏览器窗口**：
   **若出现"请进行人机验证"请点一下验证框**，看到"所有作业同步执行完毕"即成功。
   - **第一次**需要你点一次验证 + 登录（会保存到本地配置目录）。
   - **之后每次**都**静默运行（不弹窗）**：不再点验证、也不用登录，直接自动完成签到/抽奖/互动，
     完成后会弹一个"签到完成"的**通知气泡**。（验证会话约 30 天过期，过期后再点一次即可。）
